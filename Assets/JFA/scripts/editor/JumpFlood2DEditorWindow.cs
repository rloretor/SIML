...
namespace JFA.scripts.editor
{
    public class JumpFlood2DEditorWindow : JumpFloodEditorBase
    {
        ...
        {
            SourceOptions tempSourceProperties = (SourceOptions) EditorGUILayout.Popup("JFA properties source",
                (int) sourceProperties, Enum.GetNames(typeof(SourceOptions)));

            if (seedController == null || tempSourceProperties != sourceProperties)
            {
                if (sourceProperties == SourceOptions.SourceTexture)
                {
                    seedController = new JFA2DTextureDrivenSeedController();
                }
                else
                {
                    seedController = new JFA2DInputDrivenSeedController();
                }
            }
        ...
        private void DrawAndSetMaxPasses()
        {
            ...
            if (forceMaxPasses)
            {
                ...
                {
                    EditorGUILayout.HelpBox($"The algorithm must run with at least {minPasses} passes",
                        MessageType.Warning);
                }
            }
            else
